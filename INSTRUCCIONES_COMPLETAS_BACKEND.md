...
Este documento contiene las instrucciones para implementar 2 cambios importantes en el backend.
...
## 📋 Cambio 2: Filtrar Pedidos de TU MUNDO PUERTA
...
#### 2.2 Modificar Endpoint `/pedidos/all/`
...
#### 2.3 Modificar Endpoint `/pedidos/mis-pagos`

**Ubicación:** Línea ~4655
...
2. **Modificar endpoint `/all/`:**
   - Buscar `@router.get("/all/")` (línea ~142)
   - Después de `query = excluir_pedidos_web(query)`, agregar:
     ```python
     query = excluir_pedidos_tu_mundo_puerta(query)
     ```

3. **Modificar endpoint `/mis-pagos`:**
   - Buscar `@router.get("/mis-pagos")` (línea ~4655)
...
2. **Orden de aplicación:** Los filtros se aplican en orden: primero excluir web, luego excluir TU MUNDO PUERTA
...
- **Archivo único:** `api/src/routes/pedidos.py`
  ...
  - Línea ~155: Agregar filtro en `/all/`
  - Línea ~4730: Agregar filtro en `/mis-pagos`
...
- ✅ **Cambio 2 (Filtrar TU MUNDO PUERTA):** IMPLEMENTADO

Ambos cambios ya están implementados y subidos al repositorio. Este documento sirve como referencia y documentación.
